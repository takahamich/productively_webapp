...
app.get("/test", async (req, res) => { 
    // function Schedule(){
        taskModel.find({}, function (err, result) {
            ...
            if (tasksDueToday.length == 0) {
                var value = "Yay! You do not have any task due today, but you can start early on other tasks. Here is a list of tasks to start:"
                result = [value]
                ...
                let j = createTodaysSchedule(tasksToStart)
                result.push([j])
                try {
                ...
                var value =  "There are tasks that are due today! Take a look before it is too late!"
                let i = createTodaysSchedule(tasksDueToday)
                console.log("value of i", i)
                result = [value]
                if (i >= 0) {
                    result.push(["Oh no! It seems that you cannot finish all tasks that are due today. We suggest you leave out these less important tasks:"])
                    ...
                    for(var j = 0; i < tasksDueToday.length; i++) {
                        console.log("tasks due today are:", tasksDueToday[j]);
                        result.push(tasksDueToday[j])
                    }
                }
    ...
    function createTodaysSchedule(taskList) {
        ...
        for(var i = 0; i < taskList.length; i++) {
            ...
            result.push(["Start time: " + start.getHours() + ":00"])
            result.push(["Task Name: " + taskList[i].taskName ])
        }
        return (result, -1)
    }
